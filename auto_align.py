...
def align(context, bake=False, keep=False, symmetry=False):
    ...
    for m in context.selected_objects:
        ...
        normals = np.array([list(p.normal) for p in polys]) @ global_matrix[:3, :3].T
        normals = normals / np.linalg.norm(normals, axis=1, keepdims=True)

        if symmetry:
            verts = m.data.vertices
            normals_vert = (
                np.array([list(v.normal) for v in verts]) @ global_matrix[:3, :3].T
            )
            normals_vert = normals_vert / np.linalg.norm(
                normals_vert, axis=1, keepdims=True
            )
            positions_vert = (
                np.array([list(v.co) for v in verts]) @ global_matrix[:3, :3].T
            )
...
def get_symmetry_plane(normals, positions):
    ...
    normals_3 = normals_1 - 2 * plane_normals * np.sum(
        plane_normals * normals_1, axis=1
    ).reshape(-1, 1)
    ...
    plane_centers = np.sum(
        (positions_1 + positions_2)[indices] / 2 * plane_normals, axis=1
    )
    ...
    origin = (
        plane_int[(plane_int_hash == value[np.argmax(count)]).nonzero()[0][0]]
        * SYMMETRY_BUCKET_SIZE
    )
...
def get_matrix(areas, normals, fixed_axis=None):
    ...
    for index in first_indices:
        ...
        if next_indices.size > 0:
            ...
            model[1] = normals[
                np.random.choice(next_indices, p=next_areas / sum(next_areas))
            ]
        else:
            model[1] = np.zeros(3)
